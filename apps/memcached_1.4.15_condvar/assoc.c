...
// [transmem] switched from semaphores to tmcondvars
#include <tmcondvar.h>
tmcondvar_t* mx_condvar;
...
void assoc_init(const int hashtable_init) {
    ...
    //
    // [transmem] create a tmcondvar instead
    // early_sem_init();
    mx_condvar = tmcondvar_create();
...
static void assoc_start_expand(void) {
    ...
    // [transmem] switch to condvar
    // registerOnCommitHandler(ase_sempost1, NULL);
    tmcondvar_signal(mx_condvar);
}
...
static void *assoc_maintenance_thread(void *arg) {
    ...
    // [transmem] Need to be sure mx thread is able to use condvars
    ...
    while (__transaction_atomic(tm_do_run_maintenance_thread)) {
        ...
        if (!expanding) {
            ...
            // [transmem] switch from semaphore to condvar
            tmcondvar_wait(mx_condvar);
...
void stop_assoc_maintenance_thread() {
    ...
    // [transmem] switch to condvar from semaphore
    tmcondvar_signal(mx_condvar);
