...
// [transmem] switch to tmcondvar
static tmcondvar_t* slab_rebalance_condvar;
...
static void *slab_rebalance_thread(void *arg) {
    ...
    while (__transaction_atomic(tm_do_run_slab_rebalance_thread)) {
        ...
        if (__transaction_atomic(tm_slab_rebalance_signal) == 0) {
            ...
            //              the lock
            pseudo_unlock();
            // [transmem] switch to cond_wait
            tmcondvar_wait(slab_rebalance_condvar);
            pseudo_lock_spin();
        }
    }
    return NULL;
...
static enum reassign_result_type do_slabs_reassign(int src, int dst) {
    ...
    tmcondvar_signal(slab_rebalance_condvar);
...
int start_slab_maintenance_thread(void) {
    ...
    // [transmem] switch to condvar config
    slab_rebalance_condvar = tmcondvar_create();
