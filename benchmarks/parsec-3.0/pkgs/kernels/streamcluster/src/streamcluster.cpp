...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
...
#ifdef ENABLE_THREADS
  // [transmem] set up a condvar, no locks
#ifdef ENABLE_TM
  static tmcondvar_t* tm_cond = tmcondvar_create();
#else
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    while(1) {
#ifdef ENABLE_THREADS
        // [transmem] TM variant of "wait in a loop"
#ifdef ENABLE_TM
        while (1) {
    ...
    (*kcenter)++;
#ifdef ENABLE_THREADS
    // [transmem] Since the acquire and release are ifdef'd, we have really
    // ugly code.
# ifdef ENABLE_TM
    __transaction_atomic {
# else
...
# endif
    ...
    open = true;
#ifdef ENABLE_THREADS
    // [transmem] other half of the critical section boundary; move broadcast
    // into transaction
# ifdef ENABLE_TM
...
#ifdef ENABLE_THREADS
    // [transmem] See line 787... same situation here
# ifdef ENABLE_TM
    __transaction_atomic {
# else
    pthread_mutex_lock(&mutex);
