...
void AsyncImageLoader::Run()
{
    ...
    while(mCurrentFrame < mNumFrames && !mFailed)
    {
        ...
        // [transmem] use transactions here.  Note that this requires
        //            transactional STL support.
#ifdef ENABLE_TM
        __transaction_atomic {                                                      //push images to buffer
            mImageBuffer.push_back(images);
            mFGBuffer.push_back(FGImages);
        }

        tmcondvar_signal(tmCondEmpty);                                              //notify waiting threads of data
        if(mImageBuffer.size() >= mBufferSize)                                      //if buffer is full
        {   __transaction_atomic {
                tmcondvar_wait(tmCondFull);                                         //wait on full condition (wait until a request removes a set of images)
            }
        }
#else
...
bool AsyncImageLoader::GetNextImageSet(ImageSet &images, BinaryImageSet &FGMaps)
{
    ...
    {   __transaction_atomic {
            tmcondvar_wait(tmCondEmpty);
        }
    }

    __transaction_atomic {                                                          //get images from buffer
    images = mImageBuffer[0];   mImageBuffer.pop_front();
    FGMaps = mFGBuffer[0];      mFGBuffer.pop_front();
    }

    tmcondvar_signal(tmCondFull);                                                   //notify loading thread that buffer is not full
#else
    while(mImageBuffer.size() == 0)                                                 //if buffer is empty, wait until images are loaded
    {   mLock2.Lock();
        mCondEmpty.Wait();
