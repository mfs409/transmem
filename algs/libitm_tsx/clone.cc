...
_ITM_getTMCloneOrIrrevocable (void *ptr)
{
  ...
  if (ret)
    return ret;
  // [transmem] Since this TM library only supports HTM and irrevocability,
  //            we don't need to have an explicit call to become irrevocable
...
// [transmem] removed ExcludeTransaction class: it's not necessary for this
//            simpler TM library
...
_ITM_registerTMCloneTable (void *xent, size_t size)
{
  ...
  {
    // [transmem] no need to hold a lock while doing this, since it's only
